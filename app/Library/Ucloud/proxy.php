...
use App\Library\Ucloud\UCloud_Error;
...
class proxy extends ActionType
{
    private $utils;
    private $http;
    private $digest;
...
//------------------------------普通上传------------------------------
    function UCloud_PutFile($bucket, $key, $file)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::PUTFILE);
        if ($err != null) {
        ...
        list($mimetype, $err) = $this->utils->GetFileMimeType($file);
        if ($err) {
            fclose($f);
            return array("", $err);
        }
        ...
        list($data, $err) = $this->http->UCloud_Client_Call($client, $req);
        fclose($f);
...
//------------------------------表单上传------------------------------
    function UCloud_MultipartForm($bucket, $key, $file)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::POSTFILE);
        if ($err != null) {
            return array(null, $err);
        }
        ...
        $req->Header['Expect'] = '';
        $token = $this->digest->UCloud_SignRequest(null, $req, $mimetype);
        ...
        list($data, $err) = $this->http->UCloud_Client_CallWithMultipartForm($client, $req, $fields, $files);
        fclose($f);
        return array($data, $err);
    }
...
//------------------------------分片上传------------------------------
    function UCloud_MInit($bucket, $key)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::MINIT);
        if ($err != null) {
            return array(null, $err);
        }
        ...
        return $this->http->UCloud_Client_Call($client, $req);
    }
...
//@results: (tagList, err)
    function UCloud_MUpload($bucket, $key, $file, $uploadId, $blkSize, $partNumber=0)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::MUPLOAD);
        if ($err != null) {
            return array(null, $err);
        }
    ...
    function UCloud_MFinish($bucket, $key, $uploadId, $etagList, $newKey = '')
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::MFINISH);
        if ($err != null) {
            return array(null, $err);
        }
    ...
    function UCloud_MCancel($bucket, $key, $uploadId)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::MCANCEL);
        if ($err != null) {
            return array(null, $err);
        }
...
//------------------------------秒传------------------------------
    function UCloud_UploadHit($bucket, $key, $file)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::UPLOADHIT);
        if ($err != null) {
            return array(null, $err);
        }
        ...
        list($fileHash, $err) = $this->utils->UCloud_FileHash($file);
        if ($err) {
            fclose($f);
            return array(null, $err);
        }
...
//------------------------------删除文件------------------------------
    function UCloud_Delete($bucket, $key)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::DELETE);
        if ($err != null) {
            return array(null, $err);
        }
...
// @results: $url
    function UCloud_MakePrivateUrl($bucket, $key, $expires = 0)
    {
        ...
        $err = $this->utils->CheckConfig(ActionType::GETFILE);
        if ($err != null) {
            return array(null, $err);
        }
